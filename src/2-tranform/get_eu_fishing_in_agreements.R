...
# EU fleet register and fishing authorisations
...
# Select european vessels based on the authorisations CFR and UVI ---------

# Get MMSI numbers of the authorised EU fleet in MRT, SEN, GMB, GNB
register <- register |> 
  filter(cfr %in% unique(eu_authorisations$cfr) | uvi %in% unique(eu_authorisations$uvi))
...
eu_authorisations <- eu_authorisations |> 
  left_join(
    register |> distinct(cfr, mmsi),
    join_by(cfr == cfr)
  )

all(unique(gfw_data_daily$mmsi) %in% unique(eu_authorisations$mmsi)) # TRUE - not surprising because eu_authorisations$cfr was used to filter effort data

eu_fishing_in_agreements <- gfw_data_daily |> 
  left_join(
    eu_authorisations |> 
      distinct(mmsi, coastal_party_rfmo, fishing_category, fishing_category_target, fishing_category_gear, authorised_start_date, authorised_end_date), 
    by = join_by(mmsi == mmsi, eez == coastal_party_rfmo),
...
eu_fishing_out_of_agreements <- gfw_data_daily |> 
  setdiff(eu_fishing_in_agreements |> 
            select(-c(fishing_category, fishing_category_target, fishing_category_gear, authorised_start_date, authorised_end_date)))


# Save data  for clustering analysis ---------------------------------------

write.csv(eu_fishing_in_agreements,
          paste0(data_save_path, "gfw_effort_EU_vessels_within_FPA_authorisations_DAILY_2022-2024.csv"),
          row.names = FALSE)

write.csv(eu_fishing_out_of_agreements,
          paste0(data_save_path, "gfw_effort_EU_vessels_out_of_FPA_authorisations_DAILY_2022-2024.csv"),
          row.names = FALSE)
